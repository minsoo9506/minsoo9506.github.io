...
위의 분포를 보고 bayesian의 접근방식을 생각해보자. parameter $\mu$에 대한 prior를 만들어보자. *conjugacy* (prior와 posterior가 같은 분포를 갖는) 의 성질을 이용하면 Beta dist를 생각할 수 있다. prior도 beta이고 posterior도 beta dist의 모습을 보이도록 만들어준다.
