...
- 각 class 마다 $y_k({\bf x}) = {\bf w}_k^T {\bf x} + w_{k0}$ , 이를 합쳐서 표현하면

$$\textbf{y} (\textbf{x}) = \widetilde{ \textbf{W} }^T \widetilde{ {\bf x} }$$

- $\widetilde{\textbf{W}}$ : 각 컬럼이 $\widetilde{\texbf{w}}_k = ({\bf w}_{k0}, {\bf w}_k^T)$
...
$$\textbf{S}_W = \sum_{n \in C_1} (\textbf{x}_n - \textbf{m}_1)(\textbf{x}_n - \textbf{m}_1)^T + \sum_{n \in C_2} ({\bf x}_n-\textbf{m}_2)({\bf x}_n-\textbf{m}_2)^T$$
...
$$p(C_1 | {\bf x}) = \frac{p({\bf x}|C_1)p(C_1)}{p({\bf x}|C_1)p(C_1)+p({\bf x}|C_2)p(C_2)} \\= \frac{1}{1+\frac{p({\bf x}|C_2)p(C_2)}{p({\bf x}|C_1)p(C_1)}} \\ = \frac{1}{1+exp(-a) }= \sigma (a)$$
...
$$p({\bf x}|C_k) = \dfrac{1}{(2\pi)^{D/2}|\Sigma|^{1/2}}\exp\left\{-\dfrac{1}{2}({\bf x}-{\pmb \mu}_k)^T\Sigma^{-1}({\bf x}-{\pmb \mu}_k)\right\} $$
...
$${\pmb \mu}_1 = \frac{1}{N_1}\sum_{n=1}^{N}{t_n {\bf x}_n},\;\; {\pmb \mu}_2 = \frac{1}{N_2}\sum_{n=1}^{N}{(1-t_n){\bf x}_n}$$
...
$$a_k({\bf x})=\sum_{i=1}^{D}\left\{x_i\ln \mu_{ki}+(1-x_i)\ln(1-\mu_{ki})\right\}+\ln p(C_k)$$
