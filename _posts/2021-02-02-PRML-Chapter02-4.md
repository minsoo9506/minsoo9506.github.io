...
- Bernoulli distribution 예시
  ...
  $$\therefore \eta = \ln\left(\frac{\mu}{1-\mu}\right) $$
...
$$p({\pmb X}|{\pmb \eta}) = \left(\prod_{n=1}^{N}h({\pmb x}_n)\right) g({\pmb \eta})^N \exp\left\{ {\pmb \eta}^T\sum_{n=1}^{N}{\pmb u}({\pmb x}_n)\right\} $$
...
$$p({\pmb \eta}/{\pmb \chi}, v) = f({\pmb \chi}, v)g({\pmb \eta})^v \exp\{v{\pmb \eta}^T{\pmb \chi}\} \qquad{(2.229)}$$

여기에 위에서 보았던 likelihood function을 곱하여 posterior distribution을 구하면

$$p({\pmb \eta}/{\pmb X}, {\pmb \chi}, v) \propto g({\pmb \eta})^{v+N}\exp\left\{ {\pmb \eta}^T\left(\sum_{n=1}^{N}{\pmb u}({\pmb x})+v{\pmb \chi}\right)\right\}$$
...
$$% <![CDATA[
k({\pmb u})=\left\{ \begin{array}{lll} 1, & |u_i|\le 1/2, & i=1,...,D,\\
...
$$p({\pmb x}) = \frac{1}{N}\sum_{n=1}^N\frac{1}{(2\pi h^2)^{D/2}}\exp\left\{-\frac{\|{\pmb x}-{\pmb x}_n\|^2}{2h^2}\right\}$$
