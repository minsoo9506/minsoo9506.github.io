...
$$J_D = E_{x\sim p_{data}}\log D_{\theta_d}(x)+E_{z\sim p(z)}\log (1-D_{\theta_D}(G_{\theta_g}(z)))\\ \approx\frac{1}{m}\sum_{i=1}^m\log D_{\theta_D}(x_i)+\frac{1}{m}\sum_{i=1}^m\log (1-D_{\theta_d}(G_{\theta_g}(z_i)))$$ 
...
$$\min_{G}V(D^{*},G)	=E_{x\sim p_{data}(x)}[\log D^{*}(\boldsymbol{x})]+E_{z\sim p_{g}(z)}[\log\{1-D^{*}(\boldsymbol{x})\}]
	\\=E_{x\sim p_{data}(x)}\left[\log\frac{P_{data}(\boldsymbol{x})}{P_{data}(\boldsymbol{x})+P_{g}(\boldsymbol{x})}\right]+E_{x\sim p_{g}(z)}\left[\log\frac{P_{g}(\boldsymbol{x})}{P_{data}(\boldsymbol{x})+P_{g}(\boldsymbol{x})}\right]
	\\=\int_{x}p_{data}(x)\log\left[\frac{P_{data}(\boldsymbol{x})}{P_{data}(\boldsymbol{x})+P_{g}(\boldsymbol{x})}\right]dx+\int_{x}p_{g}(x)\log\left[\frac{P_{g}(\boldsymbol{x})}{P_{data}(\boldsymbol{x})+P_{g}(\boldsymbol{x})}\right]dx
	\\=-\log4+\int_{x}p_{data}(x)\log\left[\frac{p_{data}(x)}{\frac{p_{data}(x)+p_{g}(x)}{2}}\right]dx+\int_{x}p_{g}(x)\log\left[\frac{P_{g}(\boldsymbol{x})}{\frac{P_{data}(\boldsymbol{x})+P_{g}(\boldsymbol{x})}{2}}\right]dx
...
- disadvantage
  - There is no explicit rep[resentation of $p_{g}(\boldsymbol{x})$.
