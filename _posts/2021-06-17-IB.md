...
mathjax : true
date: 2021-06-17
title: "Deep Variational Information Bottelneck (ICLR 2017)"
---
...
$$I(Z,Y)-\beta I(Z,X)	\ge L \\ = \int dxdydzp(z|x)p(y|x)p(x)\log q(y|z)-\beta\int dxdzp(x)p(z|x)\log\frac{p(z|x)}{r(z)}\\ \approx\frac{1}{N}\sum_{n=1}^{N}\left[\int dz\left\{ p(z|x_{n})\log q(y_{n}|z)-\beta p(z|x_{n})\log\frac{p(z|x_{n})}{r(z)}\right\} \right]$$
