...
mathjax : true
date: 2021-06-17
title: "[Generative Model] Auto-Encoding Variational Bayes (2014)"
---
...
$$L(\boldsymbol{\theta} ,\boldsymbol{\phi};\textbf{x}^{(i)})\approx \frac{1}{2}\sum_{j=1}^{J}(1+\log(\sigma_{j}^{2(i)}) -\mu_{j}^{2(i)} - \sigma_{j}^{2(i)}) + \frac{1}{L}\sum_{l=1}^L \log p_{\boldsymbol{\theta}}(\textbf{x}^{(i)} | \textbf{z}^{(i,l)}) \\ \textbf{z}^{(i,l)} = \boldsymbol{\mu}^{(i)}+\boldsymbol{\sigma}^{(i)} \odot \boldsymbol{\epsilon}^{(l)},\; \boldsymbol{\epsilon}^{(l)} \sim N(\textbf{0},\textbf{I})$$
